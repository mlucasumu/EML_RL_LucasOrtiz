...
\chapter{Problema del Bandido de $k$-brazos}\label{capuxedtulo-1-problema-del-bandido-de-k-brazos}

%\section{\texorpdfstring{Capítulo 1: Problema del Bandido de
%\emph{k}-brazos}{Capítulo 1: Problema del Bandido de k-brazos}}

\section{Introducción}\label{introducciuxf3n}

\subsection{Descripción del Problema y
Relevancia}\label{descripcion-del-problema-y-relevancia}
...
agente es maximizar su recompensa acumulada. La toma de su decisión es
crítica: si confía únicamente en la palanca que le ha concedido mayor
ganancia económica hasta ahora (explotación), cae en el riesgo de
converger hacia una solución sub-óptima ignorando una palanca numéricamente
...
\subsection{Motivación del Trabajo}\label{motivaciuxf3n-del-trabajo}
...
\subsection{Objetivos del Informe}\label{objetivos-del-informe}
...
Documento}\label{organizacion-del-documento}
...
\textbf{Desarrollo} (\ref{desarrollo}), se sentarán las bases teóricas definiendo las distribuciones de probabilidad a las que se enfrenta
el bandido, adjuntando a la par un breve estado del arte de las
influencias bibliográficas. Seguidamente, en la sección de
\textbf{Algoritmos} (\ref{algoritmos}), se expondrá la matemática formal y el
...
\textbf{Evaluación/Experimentos} (\ref{evaluacion-y-experimentos}). Por último, los resultados son
discutidos críticamente en las \textbf{Conclusiones} (\ref{conclusiones}) para
certificar el descubrimiento de los parámetros ganadores globales.

\section{Desarrollo}\label{desarrollo}

\subsection{Contexto y Antecedentes}\label{contexto-y-antecedentes}
...
A lo largo del presente marco de validación, las recompensas \(R_t\) de cada brazo
...
\item
  ...
  estocástica de cada acción se muestrea de una distribución normal
  \(R_t \sim \mathcal{N}(\mu_a, \sigma_a^2)\), donde la verdadera medida
  subyacente de recompensa está dominada por la siguiente función de densidad de
  probabilidad:
...
\item
  ...
  estabilizar es exactamente la probabilidad de éxito intrínseca al brazo
  evaluado (\(q_*(a) = p_a\)):
  \[ P(R_t) = p_a^{R_t} (1 - p_a)^{1 - R_t} \]
\end{enumerate}

\subsection{Enfoque y Justificación
...
estructuras pre-configuradas. El presente trabajo se caracteriza
...
Acumulado} en ecosistemas tanto con recompensas continuas como discretas.
...
\begin{itemize}
    ...
    \item[\textcolor{UMUblue}\textbullet] \textbf{Métodos Epsilon y Ascenso de Gradiente (Sutton y
    ...
    \item[\textcolor{UMUblue}\textbullet] \textbf{Manejo de la Incertidumbre con
...
\subsection{\texorpdfstring{Familia 1: Lógicas de
...
El algoritmo \(\epsilon\)-Greedy (Algorithm \ref{algoritmo-greedy}) es el punto de
...
\begin{algorithm}
\caption{Algoritmo $\epsilon$-Greedy para K-Brazos}\label{algoritmo-greedy}
\begin{algorithmic}[1]
...
\For{cada paso $t=1, 2, \dots, T$}
    \State $p \leftarrow$ Valor aleatorio de Distribución $\mathcal{U}(0, 1)$
    \If{$p < \epsilon$}
...
\subsection{Familia 2: Optimismo ante la Incertidumbre
(UCB)}\label{familia-2-optimismo-ante-la-incertidumbre-ucb}

A diferencia de explorar siempre al azar, los
...
Mostramos el pseudocódigo de UCB2 en Algorithm \ref{algoritmo-ucb2}.

\begin{algorithm}
\caption{Algoritmo UCB2}\label{algoritmo-ucb2}
\begin{algorithmic}[1]
...
\For{paso de tiempo $t = K+1, \dots$}
    \State $A_t \leftarrow \arg\max_a \left[ Q(a) + \sqrt{\frac{(1 + \alpha) \ln (e \cdot t / \tau(r_a))}{2 \tau(r_a)}} \right]$ \newline
...
\subsection{Familia 3: Ascenso de Gradiente Estocástico (Softmax y
Preferencias)}\label{familia-3-ascenso-de-gradiente-estocuxe1stico-softmax-y-preferencias}

Si los algoritmos solo se fijan en los puntos
...
porcentajes (con la Ley Exponencial Suave o Gibbs). Es decir, estos métodos 
emplean una distribución de probabilidad para decidir la siguiente acción a realizar.
El pseudocódigo del algoritmo de Gradiente de Preferencias se muestra en Algorithm \ref{algoritmo-gradiente}.

\begin{algorithm}
\caption{Algoritmo de Gradiente de Preferencias}\label{algoritmo-gradiente}
\begin{algorithmic}[1]
...
\For{$t = 1, \dots, T$}
    ...
    \State $A_t \leftarrow$ Valor aleatorio de distribución de densidad $P(\cdot)$
...
\section{Evaluación y
Experimentos}\label{evaluacion-y-experimentos}
...
\textbf{Horizonte de Simulador (\emph{Steps}):} \(T = 300\) pasos
...
Para llevar a cabo las simulaciones, se definieron para cada algoritmo las
parrillas de hiperparámetros que se muestran en la Tabla \ref{tab:Parametros}.
...
Exploración-Explotación & $\epsilon$-Greedy & Tasa de exploración $\epsilon \in \{0.0, 0.01, 0.1\}$ \\ \hline
...
prismas métricos: \textbf{1. Recompensa Promedio Continua (\emph{Average
Reward}):} Traza iterativa de la estimación \(\bar{R_t}\); refleja la
capacidad del agente de percibir y adaptar ganancia bruta promedio. \textbf{2.
Ratio de Selección del Brazo Óptimo (\emph{\% Optimal Action}):}
Discrimina de forma binaria el acierto o el fallo explícito al localizar
la mejor distribución subyacente \(\arg\max q_*(a)\). Es decir, índica el porcentaje de veces que se ha seleccionado el brazo óptimo en cada paso. 
\textbf{3. Arrepentimiento Acumulado (\emph{Cumulative Regret}):} Mide la
acumulación de pérdida teórica u ``oportunidad perdida'' a nivel
temporal de decantarse por brazos subóptimos. Se busca priorizar gráficas logarítmicas y rápidamente estabilizadas (planas). \textbf{4. Histogramas Distribucionales:}
Distribuciones de las recompensas obtenidas con cada brazo, así como el número de veces que cada brazo ha sido seleccionado (\emph{Arm Statistics}).

\subsection{Análisis de Resultados}\label{analisis-de-resultados}
...
del estado del arte, englobados en la Tabla \ref{tab:Resultados}.
...
Softmax         & Temperatura $\tau = 0.5$                 & Curva modesta en continuas; catastrófico (20\% acierto) en dominios Bernoulli. \\ \hline
P. Gradiente    & Ratio de Aprendizaje $\alpha = 0.5$      & La revelación. Su matriz de distancias absolutas mitiga por completo entornos acotados y discretos (Bernoulli).  \\ \hline
UCB             & $c = 1.0$ (o $c=0.5$ Bernoulli)                    & Curvas de convergencia teórica muy seguras y logarítmicas. Rinde muy bien para incertidumbres Gausianas. \\ \hline
...
\subsection{\texorpdfstring{Resultados Familia
...
\item
  ...
  expuestos en la Figura \ref{fig:edecay_normal},
...
\item
  ...
  problema grave (visible en el aplanamiento temprano de la Figura
  \ref{fig:edecay_bernoulli}): si frenamos la caída de exploración
  demasiado pronto (fijando un mínimo inamovible de
  \(\epsilon_{min}=0.001\)), el algoritmo deja de aprender de golpe.
  Esto provoca que se estanque prematuramente y no logre igualar la
  enorme tasa de aciertos que sí consiguen otros competidores.

\end{itemize}

\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{k_brazos/img/EpsilonGreedy_Decay_Global_RegretAcumulado.png}
    \caption{Curva de Arrepentimiento Acumulado de $\epsilon$-Decay (entornos Normales).}
    \label{fig:edecay_normal}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{k_brazos/img/EpsilonGreedy_Decay_Global_RegretAcumulado_2.png}
    \caption{Estancamiento del Regret de $\epsilon$-Decay ($\epsilon_{min}=0.001$) en Bernoulli.}
    \label{fig:edecay_bernoulli}
\end{figure}

\subsection{Resultados Familia de Ascenso de
...
\item
  ...
  cuando los premios son de ``todo o nada'' (\emph{Bernoulli}). Como se
  observa detalladamente en la Figura \ref{fig:softmax_bernoulli}, en
  este último caso el algoritmo se atasca rápidamente, eligiendo la
  mejor opción apenas un 20\% de las veces y acumulando muchísimas
  pérdidas a lo largo del tiempo.
...
\item
  ...
  todos los escenarios}. Su éxito es espectacular en el
  entorno \emph{Bernoulli}, donde consigue elegir la mejor palanca casi
  el 100\% de las veces desde muy temprano. La Figura
  \ref{fig:pegradiente_bernoulli} constata cómo esto hace que sus
  pérdidas (arrepentimiento) se queden en unos pobres 3 puntos tras
  300 intentos (frente a los 70 puntos perdidos por Softmax).

\end{itemize}

\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{k_brazos/img/AscensoGradiente_SinBaseline_Global_RegretAcumulado_2.png}
    \caption{Acumulación lineal de pérdidas (Regret) de Softmax en el entorno Bernoulli.}
    \label{fig:softmax_bernoulli}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{k_brazos/img/AscensoGradiente_SinBaseline_Global_RegretAcumulado_5.png}
    \caption{Regret planificado demostrando el éxito absoluto de Gradiente de Preferencias en Bernoulli.}
    \label{fig:pegradiente_bernoulli}
\end{figure}

\subsection{Resultados Familia UCB}\label{resultados-familia-ucb}
...
\item
  ...
  sin aprender casi nada (véase la escalada incontrolable de la Figura
  \ref{fig:ucb2_bernoulli}), pero a cambio resulta ser rapidísimo y el
...
\item
  ...
  \emph{Bernoulli}), donde, tal y como se aprecia en la Figura
  \ref{fig:ucb1tuned_bernoulli}, necesita atascarse al menos 100
  intentos seguidos antes de lograr darse cuenta de cuál es el brazo
  ganador.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{k_brazos/img/UCB_UCB2_Global_RegretAcumulado_2.png}
    \caption{Fracaso rotundo y de penalización lineal de UCB2 en entornos discretos Bernoulli.}
    \label{fig:ucb2_bernoulli}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{k_brazos/img/UCB_UCB1_Global_RegretAcumulado_5.png}
    \caption{Estancamiento inicial pronunicado de UCB1-Tuned frente al engaño paramétrico Bernoulli.}
    \label{fig:ucb1tuned_bernoulli}
\end{figure}

Para ver todo esto de forma más clara, las siguientes gráficas
(sacadas de probar los algoritmos en entornos de recompensas de tipo
\emph{Normal}) muestran un resumen visual. Tal y como
...
En la Figura \ref{fig:egreedy_perf}, se diferencia claramente cómo una
...
\subsection{Resumen de Principales
...
trasladamos estos algoritmos a la vida real, estos algoritmos podrían
...
\subsection{Impacto y Reflexión en el Aprendizaje por
